...
Note that \eqref{eq:system_linear} differs from the [linear scalar homogeneous ODE](/milestone3/milestone3_odesolvers/#eqsimpleode) that we analyzed in Milestone 3. As a matter of fact, \eqref{eq:system_linear} is nonhomogeneous and the different ODEs are coupled through matrix $\mat{A}$.
...
For systems of ODEs, we require that **all** values $\lambda_i \Delta t$, for $i=1, 2, \ldots, \ndof$, lie in the stability region of the time-integration method used.
...
$$
\R^{\ndof\times\ndof}\ni \mat{A}_{\text{EBM}} = \left[ \mat{A}_{\text{EBM}} \hat{\mathbf{K}}^{1}, \mat{A}_{\text{EBM}} \hat{\mathbf{K}}^{2}, \ldots, \mat{A}_{\text{EBM}} \hat{\mathbf{K}}^{\ndof}  \right].
$$
