...
\draw[->, line width=\w, red ] ({\r * cos(\t /2) + 4},{\r * sin( \t /2 ) }) node[right=1mm, black]{\LARGE $S_{sol}(\theta_1)$} -- ({\r * cos(\t /2) },{\r * sin( \t /2 ) });
\draw[->,line width=0.5*\w, red] ({\r * cos(\t /2) },{\r * sin( \t /2 ) }) -- ({\r * cos(\t /2) + 4},{\r * sin( \t /2 ) +1}) node[right=1mm, black] {\LARGE $\alpha(\theta_1)S_{sol}(\theta_1)$};

\draw[->, decoration={snake}, decorate, red, line width=\w] ({\r * cos(\t /2 - 5) },{\r * sin( \t /2 - 5) }) -- ({\r * cos(\t /2 -5 ) + 4.05},{\r * sin( \t /2 -5)}) node[left=10mm, black, below=2mm] {\Large $S_{OLW}\sim A + B(T(\theta_1))$};

\draw[red, ->, line width = \w] ({\r * cos(\t -5)},{\r * sin( \t -5 ) })  arc[start angle = \t-5, end angle=\t+5 , radius = \r];

\draw ({\r * cos(\t)},{\r * sin( \t ) }) node[right,rotate= \t]{\Large Heat transport $\sim D(T(\theta_1)-T_{avg})$};

\draw ({\r * cos(\t/2-4)},{\r * sin( \t/2-4) }) node[left, rotate=0]{\Large with $\alpha(\theta_1), \, T(\theta_1)$};

\draw ({\r * cos(\t/2+1.5)},{\r * sin( \t/2+1.5) }) node[left, rotate=0]{\Large Latitude $\theta_1$};
